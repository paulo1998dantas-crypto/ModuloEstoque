...
O Portal assina um comprovante de curta duração para cada aplicação.  O
comprovante é validado no backend de destino e nunca é aceito pelo browser
...
def _portal_url() -> str:
    return os.environ.get(
        "ERP_PORTAL_URL", "https://ji-portal-operacional.onrender.com"
    ).strip().rstrip("/")


def portal_login_url(app_code: str, next_path: str | None) -> str:
    query = urlencode({"app": app_code, "next": normalize_next(next_path)})
    return f"{_portal_url()}/login?{query}"
...
def consume_ticket(ticket: str | None, expected_app: str) -> dict:
    """Validate a short-lived, target-bound assertion issued by the Portal."""
    ...
    if not username or len(username) > 128:
        raise ValueError("Comprovante de acesso invalido.")
    claims["next"] = normalize_next(claims.get("next"))
    claims["uid"] = user_id
