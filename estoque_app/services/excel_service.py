...
from services.estoque_service import create_or_update_sku, decimal_to_str, get_sku_by_code, to_decimal


SKU_IMPORT_COLUMNS = ["SKU", "DESCRICAO", "UNIDADE", "CATEGORIA", "LOCALIZACAO", "ESTOQUE_MINIMO"]
LABEL_IMPORT_COLUMNS = ["SKU", "QUANTIDADE"]


def _normalize_header(value):
    return str(value or "").strip().upper().replace("Ç", "C").replace("Ã", "A").replace("Í", "I")
...
def import_skus_from_excel(db, file_obj):
    ...
    ws = wb.active
    headers, header_row = _headers(ws, SKU_IMPORT_COLUMNS)
    missing = [col for col in SKU_IMPORT_COLUMNS if col not in headers]
    if missing:
    ...
    for row_number in range(header_row + 1, ws.max_row + 1):
        ...
        data = {
            "sku": raw_sku,
            "descricao": raw_desc,
            "unidade": ws.cell(row_number, headers["UNIDADE"]).value,
            "categoria": ws.cell(row_number, headers["CATEGORIA"]).value,
            "localizacao": ws.cell(row_number, headers["LOCALIZACAO"]).value,
            "estoque_minimo": ws.cell(row_number, headers["ESTOQUE_MINIMO"]).value or 0,
            "active": True,
        }
        try:
...
def create_template_files(base_dir):
    ...
    _style_header(ws)
    for width, column in zip([18, 48, 14, 24, 24, 18], "ABCDEF"):
    ...
    ws.append(SKU_IMPORT_COLUMNS)
    examples = [
        ["PAR-0001", "Parafuso sextavado M8 x 30 zincado", "UN", "Fixadores", "A1-01", 50],
        ["CAB-0012", "Cabo eletrico flexivel 2,5 mm preto", "M", "Eletrica", "B2-04", 100],
        ["CON-0100", "Conector pneumatico reto 1/4", "UN", "Pneumatica", "C3-02", 20],
    ]
    for row in examples:
        ws.append(row)
    _style_header(ws)
...
def export_stock_report(db, user, filters):
    ...
    for sku in query.order_by(SKU.sku).all():
        ...
        if to_decimal(saldo) <= 0:
            status = "ZERADO"
        elif to_decimal(saldo) <= to_decimal(sku.estoque_minimo):
            status = "BAIXO"
        else:
            status = "OK"
        ws.append([
            ...
            decimal_to_str(sku.estoque_minimo),
